...
class L2XTextExplainer:
    ...
    def __init__(self,
        ...
        if conv_ksize <= 0:
            raise ValueError("Kernel size of filters in convolution layers should be positive, but {} given".format(conv_ksize))
        ...
        if importance_sampler not in ['gumbeltopk', 'softsub']:
            raise ValueError("Only possible values for importance samper are 'gumbeltopk'," \
        ...
        if not isinstance(max_vocab_length, int):
            ...
            raise ValueError("Only avaliable values for max_vocab_length: -1 or grater 0, but {} given".format(max_vocab_length))
